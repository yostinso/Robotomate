(function($) {
  ...
  if (typeof(Device.Insteon) == "undefined") { throw("Device.Insteon not defined"); }
  ...
  var lamp_image = function (val, klass) {
    ...
    $(document.createElement('img'))
      .attr('src', Device.Insteon.Lamp.BULB_ICON)
      .attr('alt', val)
      .attr('title', val + "/" + Device.Insteon.Lamp.MAX_BRIGHT)
      .appendTo(lamp_bg_div);
    return lamp_bg_div;
  };

  Device.Insteon.Lamp = function(attributes) {
    this.parent = new Device.Insteon(attributes);
    ...
    this.parent.renderers['state'] = $.proxy(function(container, d, val) {
      ...
      if (btn_dropdown.length != 1) {
        ...
        for (var i = Device.Insteon.Lamp.MIN_BRIGHT; i <= Device.Insteon.Lamp.MAX_BRIGHT; i++) {
          $(document.createElement('li'))
            .append(
              $(document.createElement('a')).attr('href', '#').append(
                $(document.createElement('i')).addClass('icon-white').append(lamp_image(i, 'insteon_lamp_brightness_icon'))
              ).append(
                "Dim to: " + i
              ).click(
                $.proxy(
    ...
    this.parent.renderers['dim_level'] = function(container, d, val) {
      container.html(lamp_image(val, 'insteon_lamp_brightness'));
    };
    $.extend(this, this.parent);
    $.extend(this, Device.Insteon.Lamp.prototype);
  };
  Device.Insteon.Lamp.prototype.subscribe = function(remote_event_proxy) {
  ...
  Device.Insteon.Lamp.prototype.unsubscribe = function(remote_event_proxy) {
  ...
  Device.Insteon.Lamp.prototype.set_dim = function(val) {
    $.ajax({
  ...
  Device.Insteon.Lamp.MIN_BRIGHT = 1;
  Device.Insteon.Lamp.MAX_BRIGHT = 18;
  Device.Insteon.Lamp.namespace = "device";
})(jQuery);
